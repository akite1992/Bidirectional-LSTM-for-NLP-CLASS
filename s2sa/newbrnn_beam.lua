...
function generate_beam(model, initial, K, max_sent_l, source, source_features, gold)
  ...
  for t = 1, source_l do
    ...
    rnn_state_enc = out
    context[{{},t,{1,model_opt.rnn_size}}]:copy(out[#out])
  end
  rnn_state_dec = {}
  ...
  if model_opt.init_dec == 1 then
    for L = 1, model_opt.num_layers-1 do
  ...
  if model_opt.brnn == 1 then
    ...
    for t = source_l, 1, -1 do
      ...
      rnn_state_enc = out
      context[{{},t,{model_opt.rnn_size+1,model_opt.rnn_size*2}}]:add(out[#out])
    end
    if model_opt.init_dec == 1 then
      for L = 1, model_opt.num_layers-1 do
  ...
  local rnn_state_enc_top = {}
  for i = 1, #init_fwd_enc_top do
    table.insert(rnn_state_enc_top, init_fwd_enc_top[i]:zero())
  end
  local final_context = final_context_proto[{{}, {1,source_l}}]:clone() -- 1 x source_l x rnn_size
  print('number of models'.. #model)
  for t = 1, source_l do
    local top_encoder_input = {context[{{},t}]}
    append_table(top_encoder_input, rnn_state_enc_top)
    local out = model[5]:forward(top_encoder_input)
    rnn_state_enc_top = out
    final_context[{{},t}]:copy(out[#out])
  end

  rnn_state_dec[model_opt.num_layers*2-1+model_opt.input_feed]:copy(
        rnn_state_enc_top[1*2-1]:expand(K, model_opt.rnn_size))
  rnn_state_dec[model_opt.num_layers*2+model_opt.input_feed]:copy(
        rnn_state_enc_top[1*2]:expand(K, model_opt.rnn_size))
  ...
  final_context = final_context:expand(K, source_l, model_opt.rnn_size)
  ...
  while (not done) and (i < n) do
    ...
    if model_opt.attn == 1 then
      decoder_input = {decoder_input1, final_context, table.unpack(rnn_state_dec)}
    else
      decoder_input = {decoder_input1, final_context[{{}, source_l}], table.unpack(rnn_state_dec)}
    end
  ...
  if opt.score_gold == 1 then
    ...
    for t = 2, target_l do
      ...
      local decoder_input
      if model_opt.attn == 1 then
        decoder_input = {decoder_input1, final_context[{{1}}], table.unpack(rnn_state_dec)}
      else
...
function init(arg)
  ...
  final_context_proto = torch.zeros(1, MAX_SENT_L, model_opt.rnn_size)
  ...
  table.insert(init_fwd_dec, h_init_dec:clone()) -- memory cell
  table.insert(init_fwd_dec, h_init_dec:clone()) -- hidden state
  table.insert(init_fwd_enc_top, h_init_enc:clone())
  table.insert(init_fwd_enc_top, h_init_enc:clone())
