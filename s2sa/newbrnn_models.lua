...
function make_toplstm(data, opt, model, use_chars)
  assert(model == 'top_enc')
  local name = '_' .. model
  ...
  local inputs = {}
  table.insert(inputs, nn.Identity()()) -- x (batch_size x (2*rnn_size))
  for L = 1,n do
  ...
  for L = 1,n do
    ...
    if L == 1 then
      ...
      if dropout > 0 then
        x = nn.Dropout(dropout, nil, false):usePrealloc(nameL.."top_dropout",
