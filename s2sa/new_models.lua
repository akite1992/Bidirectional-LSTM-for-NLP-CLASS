...
function make_decoder_attn(data, opt, simple)
  ...
  table.insert(inputs, nn.Identity()())
  local target_t = nn.Linear(opt.rnn_size, 2*opt.rnn_size, false)(inputs[1])
  local context = inputs[2]
  --Modifications of original models:

  simple = simple or 0
  -- get attention

  local attn = nn.MM():usePrealloc("dec_attn_mm1",
                                   {{opt.max_batch_l, opt.max_sent_l_src, 2*opt.rnn_size},{opt.max_batch_l, 2*opt.rnn_size, 1}},  -- changes here
  ...
  local context_combined = nn.MM():usePrealloc("dec_attn_mm2",
                                               {{opt.max_batch_l, 1, opt.max_sent_l_src},{opt.max_batch_l, opt.max_sent_l_src, 2*opt.rnn_size}},
                                               {{opt.max_batch_l, 1, 2*opt.rnn_size}})
  ...
  if simple == 0 then  -- In this implementation simple must be 0
    context_combined = nn.JoinTable(2):usePrealloc("dec_attn_jointable",
                                                   {{opt.max_batch_l,2*opt.rnn_size},{opt.max_batch_l, opt.rnn_size}})
    ...
    context_output = nn.Tanh():usePrealloc("dec_attn_tanh",{{opt.max_batch_l,opt.rnn_size}})
                              (nn.Linear(opt.rnn_size*3,opt.rnn_size,false):usePrealloc("dec_attn_linear",
                                                                                        {{opt.max_batch_l,3*opt.rnn_size}})
                                                                           (context_combined))
  else
