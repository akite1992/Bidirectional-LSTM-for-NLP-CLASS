...
cmd:option('-brnn', 1, [[If = 1, use a bidirectional RNN. Hidden states of the fwd/bwd RNNs are summed.]])
...
cmd:option('-dropout', 0.25, [[Dropout probability. Dropout is applied between vertical LSTM stacks.]])
...
cmd:option('-print_every', 1, [[Print stats after this many batches]])
...
function train(train_data, valid_data)
  ...
  top_encoder_grad_proto = torch.zeros(opt.max_batch_l, opt.max_sent_l, opt.rnn_size)
  ...
  final_context_proto = torch.zeros(opt.max_batch_l, opt.max_sent_l, opt.rnn_size)
  ...
  top_encoder_clones=clone_many_times(top_encoder,opt.max_sent_l_src)
  for i = 1, opt.max_sent_l_src do
    ...
    if top_encoder_clones[i].apply then
      top_encoder_clones[i]:apply(function(m) m:setReuse() end)
      if opt.prealloc == 1 then top_encoder_clones[i]:apply(function(m) m:setPrealloc() end) end
    end
    if opt.brnn == 1 then
  ...
  if opt.gpuid >= 0 then
    ...
    if opt.gpuid2 >= 0 then
      ...
      context_proto = context_proto:cuda()
      final_context_proto=final_context_proto:cuda()
      encoder_grad_proto = encoder_grad_proto:cuda()
      encoder_bwd_grad_proto=encoder_bwd_grad_proto:cuda()
      top_encoder_grad_proto=top_encoder_grad_proto:cuda()
    end
  end
  ...
  init_fwd_enc_top={}
  init_bwd_enc_top={}
  table.insert(init_fwd_enc_top, h_init:clone())
  table.insert(init_fwd_enc_top, h_init:clone())
  table.insert(init_bwd_enc_top, h_init:clone())
  table.insert(init_bwd_enc_top, h_init:clone())

  for L = 1, opt.num_layers-1 do
  ...
  function train_batch(data, epoch)
    ...
    for i = 1, data:size() do
      ...
      local top_encoder_grads=top_encoder_grad_proto[{{1,batch_l},{1,source_l}}]
      if opt.gpuid >= 0 then
        cutorch.setDevice(opt.gpuid)
      end
      ...
      for t = 1, source_l do
        ...
        rnn_state_enc[t] = out
        context[{{},t,{1,opt.rnn_size}}]:copy(out[#out])
      end
      ...
      if opt.brnn == 1 then
        ...
        for t = source_l, 1, -1 do
          ...
          context[{{},t,{opt.rnn_size+1,opt.rnn_size*2}}]:copy(out[#out])
        end
      end


      local rnn_state_enc_top=reset_state(init_fwd_enc_top, batch_l, 0)
      local final_context=final_context_proto[{{1,batch_l},{1,source_l}}]
      -- forward top encoder
      for t = 1, source_l do
        top_encoder_clones[t]:training()
        local top_encoder_input = {context[{{},t}]}
        append_table(top_encoder_input, rnn_state_enc_top[t-1])
        local out = top_encoder_clones[t]:forward(top_encoder_input)
        rnn_state_enc_top[t] = out
        final_context[{{},t}]:copy(out[#out])
      end
      ...
      if opt.init_dec == 1 then
        ...
        rnn_state_dec[0][opt.num_layers*2-1+opt.input_feed]:copy(rnn_state_enc_top[source_l][1*2-1])
        rnn_state_dec[0][opt.num_layers*2+opt.input_feed]:copy(rnn_state_enc_top[source_l][1*2])
      end
      ...
      for t = 1, target_l do
        ...
        local decoder_input
        if opt.attn == 1 then
          decoder_input = {target[t], final_context, table.unpack(rnn_state_dec[t-1])}
        else
      ...
      for t = target_l, 1, -1 do
        ...
        if opt.attn == 1 then
          --encoder_grads:add(dlst[2][{{},{},{1,opt.rnn_size}}])
          --if opt.brnn == 1 then
            --encoder_bwd_grads:add(dlst[2][{{},{},{opt.rnn_size+1,2*opt.rnn_size}}])
          --end
          top_encoder_grads:add(dlst[2])
        else
      ...
      local drnn_state_enc_top=reset_state(init_bwd_enc_top,batch_l)
      if opt.init_dec == 1 then
        for L = 1, 1 do
          drnn_state_enc_top[L*2-1]:copy(drnn_state_dec[opt.num_layers*2-1])
          drnn_state_enc_top[L*2]:copy(drnn_state_dec[opt.num_layers*2])
        end
      end
      for t = source_l, 1, -1 do
        local top_encoder_input = {context[{{},t}]}
        append_table(top_encoder_input, rnn_state_enc_top[t-1])
        if opt.attn == 1 then
          drnn_state_enc_top[#drnn_state_enc_top]:add(top_encoder_grads[{{},t}])
        end
        local dlst = top_encoder_clones[t]:backward(top_encoder_input, drnn_state_enc_top)
        for j = 1, #drnn_state_enc_top do
          drnn_state_enc_top[j]:copy(dlst[j+1])
        end
        encoder_grads[{{},t}]:add(dlst[1][{{},{1,opt.rnn_size}}])
        encoder_bwd_grads[{{},t}]:add(dlst[1][{{},{opt.rnn_size+1,2*opt.rnn_size}}])
      end
      ...
      grad_norm = grad_norm + grad_params[1]:norm()^2+grad_params[5]:norm()^2
      if opt.brnn == 1 then
  ...
  for epoch = opt.start_epoch, opt.epochs do
    ...
    if epoch % opt.save_every == 0 then
      ...
      if opt.brnn == 0 then
        torch.save(savefile, {{encoder, decoder, generator,top_encoder}, opt})
      else
        torch.save(savefile, {{encoder, decoder, generator, encoder_bwd, top_encoder}, opt})
      end
    end
  end
  -- save final model
  ...
  if opt.brnn == 0 then
    torch.save(savefile, {{encoder:double(), decoder:double(), generator:double(),top_encoder:double()}, opt})
  else
    torch.save(savefile, {{encoder:double(), decoder:double(), generator:double(),
          encoder_bwd:double(),top_encoder:double()}, opt})
  end
end
...
function eval(data)
  ...
  for i = 1, data:size() do
    ...
    local rnn_state_enc_top = reset_state(init_fwd_enc_top, batch_l)
    local final_context = final_context_proto[{{1, batch_l}, {1, source_l}}]
    -- forward prop encoder
    for t = 1, source_l do
      local top_encoder_input = {context[{{},t}]}
      append_table(top_encoder_input, rnn_state_enc_top)
      local out = top_encoder_clones[1]:forward(top_encoder_input)
      rnn_state_enc_top = out
      final_context[{{},t}]:copy(out[#out])
    end

    if opt.init_dec == 1 then
      rnn_state_dec[opt.num_layers*2-1+opt.input_feed]:copy(rnn_state_enc_top[1*2-1])
      rnn_state_dec[opt.num_layers*2+opt.input_feed]:copy(rnn_state_enc_top[1*2])
    end
    ...
    for t = 1, target_l do
      local decoder_input
      if opt.attn == 1 then
        decoder_input = {target[t], final_context, table.unpack(rnn_state_dec)}
      else
...
function main()
  ...
  if opt.train_from:len() == 0 then
    ...
    top_encoder=make_toplstm(valid_data,opt,'top_enc',opt.use_chars_enc)
  else
    ...
    print('number of models from checkpoint'..#model)
  end
  ...
  table.insert(layers, top_encoder)
